...
jobs:
  sync_content:
    ...
    steps:
      - name: 检出代码
        # *** 修正：将 actions:checkout@v4 改为 actions/checkout@v4 ***
      ...
      - name: 执行同步脚本
        ...
        run: |
          ...
          # 构造 HTML/Markdown 块 (使用单行定义，避免 YAML 冲突)
          # max-height: 200px (约 10 行)
          HTML_START='<details><summary>点击展开最新同步内容 ('
          HTML_END=")</summary><div style=\"max-height: 200px; overflow: auto; border: 1px solid #ccc; padding: 10px;\">\n\n\`\`\`\n"
          HTML_CODE_END="\n\`\`\`\n\n</div></details>"
          
          # 组合最终要插入的内容
          NEW_CONTENT_BLOCK="${HTML_START}${SYNC_TIME}${HTML_END}${GIST_CONTENT_BLOCK}${HTML_CODE_END}"
          
          # 准备要插入的内容 (转义换行符，以便 sed 'a' 命令使用)
          INSERT_CONTENT_ESCAPED=$(printf "%s" "$NEW_CONTENT_BLOCK" | sed -e 's/\\/\\\\/g' -e ':a' -e 'N' -e '$!ba' -e 's/\n/\\n/g')
